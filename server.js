...
app.post("/api/login", async (req, res) => {
  try {
    ...
    res.json({ token, message: "Iser login successfully" });
  } catch (err) {
    res.status(500).json({ message: "Server error", error: err.message });
  }
});
...
//     const resetLink = `http://localhost:3000/resetpasswordpage/${token}`;
...
//       from: `"Lawsum" <${process.env.EMAIL_USER}>`,
//       to: email,
...
app.post("/api/forgot-password", async (req, res) => {
  try {
    ...
    const plainText = `Hello ${
      user.name || "User"
    },\n\nYou requested a password reset for your LawSumm account.\n\nReset your password here: ${resetLink}\n\nIf you didn't request this, just ignore this email.\n\n© ${new Date().getFullYear()} LawSumm`;
      ...
      <html>
        ...
        <body style="font-family: Arial, sans-serif; background-color: #f9f9f9; color: #333; padding: 20px;">
          <div style="max-width: 600px; margin: auto; background-color: #fff; padding: 30px; border-radius: 10px; box-shadow: 0 0 10px rgba(0,0,0,0.1);">
            ...
            <p>We received a request to reset your password for your LawSumm account.</p>
            ...
            <hr />
            <p style="font-size: 12px; color: #999;">© ${new Date().getFullYear()} LawSumm. All rights reserved.</p>
          </div>
        </body>
      </html>
    `;
    ...
    await transporter.sendMail({
      from: `"LawSumm Support" <${process.env.EMAIL_USER}>`, // sender email must match Gmail account
      to: email,
      replyTo: process.env.EMAIL_USER,
      subject: "Reset Your Password - LawSumm",
      text: plainText,
      html: htmlContent,
    });
